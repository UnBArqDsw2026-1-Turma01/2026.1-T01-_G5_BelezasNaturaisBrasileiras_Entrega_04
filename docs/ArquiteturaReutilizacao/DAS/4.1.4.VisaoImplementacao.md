...
## Componentes de Build - Diagrama Conceitual (de Alto Nível)
...
## Comprobatório de Execução: Estilo Arquitetural N-Camadas

Conforme exigido nas instruções da avaliação, apresentamos o fluxo de uma requisição real demonstrando o respeito estrito às responsabilidades de cada camada no backend (`interface` → `application` → `domain` → `infrastructure`).

O exemplo abaixo rastreia o fluxo de **"Solicitação de Inscrição em Trilha"**:

**1. Camada de Interface (Controller)**
Recebe a requisição HTTP, extrai o usuário via JWT e delega a lógica imediatamente para a Application, sem acessar banco de dados ou regras negociais.
```typescript
...
**2. Camada de Aplicação (UseCase / Facade)**
Orquestra o processo: invoca regras do Domínio e coordena a gravação na Infraestrutura.
```typescript
@Injectable()
export class SolicitarInscricaoUseCase {
  ...
  async execute(usuarioId: string, trilhaId: string): Promise<Inscricao> {
    // ...validações preliminares (como verificar se a trilha existe)...
    
    // Cria entidade rica de domínio utilizando o construtor
    const novaInscricao = new Inscricao(randomUUID(), trilhaId, usuarioId); 
    
    // Delega ao repositório (apenas via interface IInscricaoRepository)
    return this.inscricaoRepository.create(novaInscricao);
  }
}
```

**3. Camada de Infraestrutura (Repository)**
Implementa o contrato exigido pela interface do domínio e realiza a persistência real usando Prisma ORM e os Mappers de dados.
```typescript
...
```
Essa separação garante a testabilidade de `SolicitarInscricaoUseCase` usando mocks, validando a arquitetura proposta.

**4. Execução Prática e Comprobatórios de Fluxo:**

A execução do fluxo N-Camadas pode ser comprovada de duas formas no projeto:

* **Execução via Requisição de Integração (cURL / HTTP):**
  Uma chamada para criar uma inscrição de trilha ativa todas as camadas do backend sequencialmente:
  ```bash
  # Efetuando a chamada na Camada de Interface (Controller)
  $ curl -X POST http://localhost:3000/inscricoes/trilha/9e5c544e-1234-5678-abcd-123456789abc \
    -H "Authorization: Bearer <JWT_TOKEN>"

  # Resposta HTTP 201 (Created) após passar por UseCase, Repositório e Prisma:
  {
    "id": "7ca6b8c9-0213-4c9f-8a1e-848fb1859c23",
    "usuarioId": "3b2a1c0d-e4f5-6a7b-8c9d-0e1f2a3b4c5d",
    "trilhaId": "9e5c544e-1234-5678-abcd-123456789abc",
    "status": "PENDING",
    "dataSolicitacao": "2026-06-20T23:15:00.000Z",
    "codigoCheckin": "123456"
  }
  ```

* **Execução via Testes Unitários de Camada (Jest):**
  Ao rodar o comando abaixo, o framework de testes executa as camadas de aplicação (`UseCase` e `Facade`) mockando o repositório de infraestrutura:
  ```bash
  $ npx jest src/modules/inscricoes/application/InscricaoFacade.spec.ts --verbose
  ```
  **Resultado da Execução no Console:**
  ```text
  PASS src/modules/inscricoes/application/InscricaoFacade.spec.ts
    InscricaoFacade
      solicitar()
        ✓ deve delegar ao SolicitarInscricaoUseCase com os parâmetros corretos (2 ms)
        ✓ deve propagar ConflictException quando usuário já está inscrito (5 ms)
        ✓ deve propagar NotFoundException quando trilha não existe (4 ms)
      aceitar()
        ✓ deve delegar ao AceitarInscricaoUseCase com os parâmetros corretos (1 ms)
        ✓ deve propagar ForbiddenException quando usuário não é o organizador (1 ms)
      rejeitar()
        ✓ deve delegar ao RejeitarInscricaoUseCase com os parâmetros corretos
        ✓ deve propagar ForbiddenException quando usuário não é o organizador
      fazerCheckin()
        ✓ deve delegar ao FazerCheckinUseCase com os parâmetros corretos (1 ms)
        ✓ deve propagar BadRequestException quando código é inválido (1 ms)
        ✓ deve propagar ForbiddenException quando usuário não é o organizador
      minhas()
        ✓ deve delegar ao ListarInscricoesUseCase.minhas() com o usuarioId correto (1 ms)
        ✓ deve retornar array vazio quando usuário não tem inscrições
...
| Integrante | Revisão |
| :--------- | :------ |
| [Miguel Arthur](https://github.com/zlimaz) | A documentação da infraestrutura do repositório está clara. A demonstração do fluxo N-Camadas ilustra as responsabilidades corretas do Controller, UseCase e Repository na funcionalidade de inscrições em trilhas, validando o estilo arquitetural. O senso crítico sobre testes unitários reflete bem as decisões do grupo. |
...
| :----- | :--------- | :------------------------------------------------------------------------------------ | :----------------------------------------------------- | :--- | :--- |
| `1.0`  | 11/06/2026 | Criação da visão de implementação: monorepo, estrutura, testes, workflow              | [Vitor Hoffmann](https://github.com/vitor-hoffmann) e [Ana Pfeilsticker](https://github.com/ana-pfeilsticker) | [Antonio Carvalho](https://github.com/antonioscarvalho) | Revisão e validação da estrutura do monorepo e mapeamento de módulos de implementação. |
