...
graph TD
  subgraph backend["Backend — NestJS 11"]
    subgraph mod["6 módulos verticais"]
      iface["interface<br/>Controllers · Guards · Proxies HTTP"]
      app["application<br/>Use Cases · Facades · DTOs"]
      infra["infrastructure<br/>Repositórios Prisma · Factories · Adapters externos"]
      domain["domain<br/>Entidades · Interfaces · Regras de Negócio"]
    end
    shared["shared<br/>PrismaService · SupabaseProvider"]
  end

  iface -->|"chama"| app
  app -->|"depende de interfaces de"| domain
  infra -->|"implementa interfaces de"| domain
  iface -.->|"NestJS DI"| infra
  infra --> shared
```
...
## Diagrama de Classes — Domínio Central
...
classDiagram
  ...
  class ITrilhaRepository {
    <<interface>>
    +findById(id) Trilha
    +findAll(filter) Trilha[]
    +save(trilha) Trilha
  }

  User "1" --> "n" Trilha : organiza
  User "1" --> "n" Inscricao : solicita
  User "1" --> "n" Badge : recebe
  Trilha "1" --> "n" Inscricao : tem
  Trilha "1" --> "n" Badge : gera
  IUserRepository ..> User : retorna
  ITrilhaRepository ..> Trilha : retorna
```
...
| :----- | :--------- | :-------------------------------------------------------------------------------------- | :-------------------------------------------------- | :------ | :---------------------- |
| `1.0`  | 11/06/2026 | Criação da visão lógica: pacotes, classes, catálogo GoF, rastreabilidade, senso crítico | [Vitor Hoffmann](https://github.com/vitor-hoffmann) | —       | —                       |
