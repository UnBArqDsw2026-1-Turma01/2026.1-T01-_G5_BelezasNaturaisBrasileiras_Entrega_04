...
O **Decorator** é um padrão estrutural que adiciona responsabilidades a um objeto dinamicamente, fornecendo uma alternativa flexível ao uso de subclasses. É útil quando você deseja estender a funcionalidade de objetos sem criar novas subclasses.

Este padrão permite envolver objetos em decoradores que adicionam novas funcionalidades, mantendo a interface original intacta.

## Quando Aplicar?

- Quando adicionar responsabilidades a objetos individuais dinamicamente
- Quando extensões através de subclasses é impraticável
- Quando as responsabilidades podem ser removidas ou combinadas
- Quando um grande número de subclasses independentes seria criado
- Quando você deseja adicionar funcionalidades como logging, cache ou validação transparentemente
...
> Refactoring Guru. Decorator. Disponível em: https://refactoring.guru/design-patterns/decorator. Acesso em: 18 mai. 2026.
