...
O **Template Method** é um padrão comportamental que define o esqueleto de um algoritmo em uma operação de classe base, deixando que subclasses preencham os passos específicos. É útil quando múltiplas classes têm um algoritmo similar com variações em etapas específicas.

Este padrão promove reutilização de código através de um template que define a estrutura enquanto permite que subclasses implementem passos específicos.

## Quando Aplicar?

- Quando você tem múltiplas classes com algoritmos similares
- Quando o algoritmo tem uma estrutura comum mas implementações diferentes
- Quando você deseja evitar duplicação de código
- Quando quer controlar pontos de extensão em um algoritmo
- Quando a estrutura do algoritmo é mais importante que os detalhes
...
> Refactoring Guru. Template Method. Disponível em: https://refactoring.guru/design-patterns/template-method. Acesso em: 18 mai. 2026.
