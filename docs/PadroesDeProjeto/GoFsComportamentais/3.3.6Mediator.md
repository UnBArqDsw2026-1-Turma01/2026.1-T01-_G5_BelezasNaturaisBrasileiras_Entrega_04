...
1.  **`TrailLifecycleMediatorService`**: Atua como o centro nervoso da operação. Ele mantém uma lista de handlers e os executa sequencialmente. Isso evita que a entidade `Trilha` ou o `UseCase` fiquem superlotados com lógica de badges, notificações e presença.
2.  **`Handlers` (Badge, Attendance, etc.)**: São os "colegas" no padrão. Eles recebem os dados do mediador, executam sua lógica específica e retornam o controle. Eles são altamente reutilizáveis e fáceis de testar isoladamente.
3.  **`ILifecycleHandler`**: Garante que o mediador possa lidar com qualquer nova funcionalidade futura de forma extensível (Plugin Architecture).
...
| `1.1`  | 21/05/2026 | Adição do diagrama de modelagem juntamente com a documentação geral.                                                            | [Mateus Magno](http://github.com/mtsmmgn0)       |
